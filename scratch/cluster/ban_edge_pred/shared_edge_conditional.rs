...
fn run<ER: graph::Edge>(
    label: &str,
    clusters: Vec<search::dsl::ClusterOps<(), ER>>,
    target: &graph::Graph<(), ER>,
    expected: usize,
) {
    let Search::Resolved(r) = compile::<(), ER>(clusters).unwrap()
    else { panic!("unexpected bound") };
    let query = r.into_query();
    let indexed = target.index(RevCsr);
    let n = Seq::search(&query, &indexed).collect::<Vec<_>>().len();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    expect_reject("ban pred on same-slot (undir)", compile::<i32, edge::Undir<i32>>(search![
        get(Mono) { N(0) ^ (N(1) ^ N(2)) },
        ban(Mono) { n(0) & E().test(|v: &i32| *v > 10) ^ n(1) }
    ]));

    expect_reject("ban val on same-slot (undir)", compile::<i32, edge::Undir<i32>>(search![
        get(Mono) { N(0) ^ N(1) },
    ...
    expect_reject("undir: ^ covers only slot → % dead", compile::<(), edge::Undir<()>>(search![
        get(Mono) { N(0) ^ N(1) },
        ban(Mono) { n(0) % n(1) }
    ]));

    expect_reject("dir: >> + << covers both slots → % dead", compile::<(), edge::Dir<()>>(search![
        get(Mono) { N(0) >> N(1) },
        get(Mono) { n(0) << n(1) },
        ban(Mono) { n(0) % n(1) }
    ]));
    ...
    expect_reject("dir: SubIso get >>, ban <<", compile::<(), edge::Dir<()>>(search![
        get(SubIso) { N(0) >> N(1) },
        ban(Mono) { n(0) << n(1) }
    ]));
