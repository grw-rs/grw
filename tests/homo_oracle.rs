...
fn gen_case(rng: &mut Rng) -> Case {
    ...
    let morphs = [Morphism::Mono, Morphism::Homo, Morphism::Epi];
    let cluster_morphism: Vec<Morphism> = (0..cluster_count).map(|_| morphs[rng.below(3) as usize]).collect();
    Case { target_n, target_edges, pattern_n, pattern_edges, cluster_of, cluster_morphism }
}

fn oracle(case: &Case) -> BTreeSet<Vec<u32>> {
...
fn build_and_run(case: &Case) -> Option<(BTreeSet<Vec<u32>>, usize, usize, usize)> {
    ...
    let g: grw::graph::Undir0 = gdsl::from_fragment::<(), GER>(ops).unwrap();
...
fn fuzz_mixed_morphisms_vs_oracle() {
    ...
    println!("oracle fuzz: {ran} cases verified");
    assert!(ran > 400);
}
