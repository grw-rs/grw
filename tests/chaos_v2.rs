...
trait ChaosEdge: graph::Edge<Val: Clone + Default> + Sized {
    ...
    <Graph<(), ER> as TryFrom<(Vec<(Id, ())>, Vec<(ER::Def, ())>)>>::Error: std::fmt::Debug,
{
    ...
    let actual_hist = degree_histogram::<ER, _>(graph.to_vecs());
    let expected_hist = degree_histogram::<ER, _>(shadow.to_vecs());
