...
impl<NV, EV> HasKey<NV, edge::Anydir<EV>> for edge::anydir::E<Id> {
    fn has_in(self, graph: &Graph<NV, edge::Anydir<EV>>) -> bool {
        let (nr, slot) = self.into();
        graph.edges_between(*nr.n1(), *nr.n2())
            .any(|(s, _)| s == slot)
    }
...
impl<NV, EV> GetKey<NV, edge::Anydir<EV>> for edge::anydir::E<Id> {
    ...
    fn get_from(self, graph: &Graph<NV, edge::Anydir<EV>>) -> Option<&edge::AnyVal<EV>> {
        let (nr, slot) = self.into();
        graph.edges_between(*nr.n1(), *nr.n2())
            .find(|(s, _)| *s == slot)
            .map(|(_, v)| v)
    }
    fn get_mut_from(self, graph: &mut Graph<NV, edge::Anydir<EV>>) -> Option<&mut edge::AnyVal<EV>> {
        let (nr, slot) = self.into();
        let n1 = *nr.n1();
        let n2 = *nr.n2();
        let Graph { nodes, edges, .. } = graph;
