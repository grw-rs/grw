...
/// Edge value typestate — carries the slot-specific value (not the full ER::Val).
/// The direction operator (`^`, `>>`, `<<`) wraps it via `SlotVal::wrap_slot_val`.
pub struct HasRawVal<V>(pub(crate) V);

/// Marker: edge value typestates that carry the full `ER::Val` (or default to it).
/// `()` and `HasVal<T>` implement this; `HasRawVal<V>` does not.
/// Used to prevent overlap between generic `EV: IntoVal` impls and `HasRawVal`-specific ones.
pub trait IsFullVal {}
impl IsFullVal for () {}
impl<T> IsFullVal for HasVal<T> {}
...
macro_rules! for_each_dir {
    ($mac:ident ! ($($arg:tt)+)) => {
        $mac!($($arg)+, Src, DirSlot, Shr, shr);
        $mac!($($arg)+, Tgt, DirSlot, Shl, shl);
        $mac!($($arg)+, Und, UndirSlot, BitXor, bitxor);
    };
    ($mac:ident ! ()) => {
        $mac!(Src, DirSlot, Shr, shr);
        $mac!(Tgt, DirSlot, Shl, shl);
        $mac!(Und, UndirSlot, BitXor, bitxor);
    };
}
