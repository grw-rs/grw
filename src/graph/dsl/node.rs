...
use super::{HasRawVal, HasVal, IsFullVal, IntoVal, LocalId};
use crate::graph;
...
macro_rules! impl_undir_op {
    ($NodeTy:ty, $V:ident, $Dir:ident, $_SlotDir:ident, $Op:ident, $op:ident) => {
        impl<NV, $V, EV: IntoVal<ER::Val> + IsFullVal, ER: graph::Edge + $Dir, RHS: IntoOp<NV, ER>>
    ...
    ($NodeTy:ty, $Dir:ident, $_SlotDir:ident, $Op:ident, $op:ident) => {
        impl<NV, EV: IntoVal<ER::Val> + IsFullVal, ER: graph::Edge + $Dir, RHS: IntoOp<NV, ER>>
            $Op<RHS> for UndirPending<$NodeTy, edge::Edge<EV, NV, ER>>
        {
            type Output = $NodeTy;
            fn $op(self, rhs: RHS) -> $NodeTy {
...
// HasRawVal variants: Node & E().val(slot_val) >> / << / ^ Target
// Uses SlotVal to wrap the raw value into ER::Val.
macro_rules! impl_rawval_undir_op {
    ($NodeTy:ty, $V:ident, $Dir:ident, $SlotDir:ident, $Op:ident, $op:ident) => {
        impl<NV, $V, V_, ER: graph::Edge + $Dir + SlotVal<$SlotDir, SlotType = V_>, RHS: IntoOp<NV, ER>>
    ...
    ($NodeTy:ty, $Dir:ident, $SlotDir:ident, $Op:ident, $op:ident) => {
        impl<NV, V_, ER: graph::Edge + $Dir + SlotVal<$SlotDir, SlotType = V_>, RHS: IntoOp<NV, ER>>
            $Op<RHS> for UndirPending<$NodeTy, edge::Edge<HasRawVal<V_>, NV, ER>>
        {
            type Output = $NodeTy;
            fn $op(self, rhs: RHS) -> $NodeTy {
...
impl_bitand_rawval_pending!(Node<NV, V, ER>, V);
impl_bitand_rawval_pending!(Ref<NV, ER>);
