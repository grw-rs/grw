...
impl<A: smallvec::Array + 'static> Val for smallvec::SmallVec<A> where A::Item: Val {
    fn fields() -> &'static [FieldInfo] { &[] }
    fn field_type() -> FieldType {
        FieldType::Array(<A::Item as Val>::field_type)
    }
    fn layout_hash() -> u64 {
        let mut h = FNV_OFFSET;
        h = fnv_hash_byte(h, 15u8);
        h = fnv_hash_u64(h, <A::Item as Val>::layout_hash());
        h = fnv_hash_u64(h, A::size() as u64);
        h
    }
