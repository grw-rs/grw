use super::*;

#[derive(serde::Serialize, serde::Deserialize)]
#[serde(bound(
    serialize = "NV: serde::Serialize, E::Slot: serde::Serialize, E::Val: serde::Serialize",
    deserialize = "NV: serde::de::DeserializeOwned, E::Slot: serde::de::DeserializeOwned, E::Val: serde::de::DeserializeOwned",
))]
pub struct MGraph<NV, E: Edge> {
    pub(crate) nodes: Nodes<NV>,
    pub(crate) edges: Edges<E>,
    pub(crate) degrees: Vec<(Id, IdSet<id::N>)>,
}

impl<NV: Clone, E: Edge> Clone for MGraph<NV, E> where E::Slot: Clone, E::Val: Clone {
    fn clone(&self) -> Self {
        MGraph { nodes: self.nodes.clone(), edges: self.edges.clone(), degrees: self.degrees.clone() }
    }
}

impl<NV, E: Edge> MGraph<NV, E> {
    pub fn get<K: GetKey<NV, E>>(&self, key: K) -> Option<&K::Val> {
        key.get_from(self)
    }
    pub fn get_mut<K: GetKey<NV, E>>(&mut self, key: K) -> Option<&mut K::Val> {
        key.get_mut_from(self)
    }
    pub fn has<K: HasKey<NV, E>>(&self, key: K) -> bool {
        key.has_in(self)
    }
    pub fn node_count(&self) -> usize {
        self.nodes.len()
    }
    pub fn edge_count(&self) -> usize {
        self.edges.len()
    }

    pub fn node_iter(&self) -> impl Iterator<Item = (id::N, &NV)> {
        self.nodes.iter()
    }

    pub fn edge_iter(&self) -> impl Iterator<Item = (E::Def, &E::Val)> {
        self.edges.iter()
    }

    pub fn neighbor_ids(&self, n: id::N) -> Option<impl Iterator<Item = id::N> + '_> {
        self.nodes.neighbor_ids(n)
    }

    pub fn neighbors(&self, n: id::N) -> Option<impl Iterator<Item = (id::N, E::Slot, &E::Val)> + '_> {
        let node = self.nodes.get_node(n)?;
        Some(node.adj.entries_iter().map(move |(nb, eid)| {
            let rec = self.edges.store[*eid as usize].as_ref().expect("edge record for adjacency entry");
            // Adjust slot for query direction: if n is n2 (not n1), reverse the slot.
            // Edge (n1, n2, Src) means n1→n2. From n2's perspective, that's incoming (Tgt).
            let slot = if n == rec.n1 { rec.slot } else { E::reverse_slot(rec.slot) };
            (nb, slot, &rec.val)
        }))
    }

    /// All simple paths from `from` to `to` following edges accepted by `edge_pred`.
    /// Returns each path as a Vec of node IDs (including start and end).
    /// Monotonic: no node revisited (injective/simple paths only).
    pub fn all_simple_paths(
        &self,
        from: id::N,
        to: id::N,
        edge_pred: impl Fn(E::Slot, &E::Val) -> bool,
        max_len: usize,
    ) -> Vec<Vec<id::N>> {
        enum Op { Visit(id::N), Unvisit }
        let mut results = Vec::new();
        let mut stack: Vec<Op> = Vec::new();
        let mut visited = collections::IdSet::<id::N>::default();
        let mut path: Vec<id::N> = Vec::new();

        stack.push(Op::Visit(from));

        while let Some(op) = stack.pop() {
            match op {
                Op::Unvisit => {
                    if let Some(n) = path.pop() { visited.remove(&n); }
                }
                Op::Visit(cur) => {
                    if visited.contains_key(&cur) { continue; }
                    visited.insert(&cur);
                    path.push(cur);
                    if path.len() > max_len {
                        path.pop();
                        visited.remove(&cur);
                        continue;
                    }
                    let Some(nbs) = self.neighbors(cur) else {
                        stack.push(Op::Unvisit);
                        continue;
                    };
                    stack.push(Op::Unvisit);
                    let mut pushed = smallvec::SmallVec::<[id::N; 8]>::new();
                    for (nb, slot, ev) in nbs {
                        if !edge_pred(slot, ev) { continue; }
                        if nb == to {
                            let mut p = path.clone();
                            p.push(nb);
                            results.push(p);
                        } else if !visited.contains_key(&nb) && !pushed.contains(&nb) {
                            pushed.push(nb);
                            stack.push(Op::Visit(nb));
                        }
                    }
                }
            }
        }
        results
    }

    /// Shortest path from `from` to `to` using Dijkstra's algorithm.
    /// `weight_fn` extracts a non-negative cost from (slot, edge_value).
    /// Returns (total_cost, path) or None if unreachable.
    pub fn shortest_path(
        &self,
        from: id::N,
        to: id::N,
        weight_fn: impl Fn(E::Slot, &E::Val) -> Option<u32>,
    ) -> Option<(u32, Vec<id::N>)> {
        use std::cmp::Reverse;
        use std::collections::{BinaryHeap, HashMap};

        let mut dist: HashMap<id::N, u32> = HashMap::new();
        let mut prev: HashMap<id::N, id::N> = HashMap::new();
        let mut heap: BinaryHeap<Reverse<(u32, id::N)>> = BinaryHeap::new();

        dist.insert(from, 0);
        heap.push(Reverse((0, from)));

        while let Some(Reverse((cost, cur))) = heap.pop() {
            if cur == to {
                let mut path = vec![to];
                let mut n = to;
                while let Some(&p) = prev.get(&n) {
                    path.push(p);
                    n = p;
                }
                path.reverse();
                return Some((cost, path));
            }
            if cost > *dist.get(&cur).unwrap_or(&u32::MAX) { continue; }
            let Some(nbs) = self.neighbors(cur) else { continue };
            for (nb, slot, ev) in nbs {
                let Some(w) = weight_fn(slot, ev) else { continue };
                let new_cost = cost + w;
                if new_cost < *dist.get(&nb).unwrap_or(&u32::MAX) {
                    dist.insert(nb, new_cost);
                    prev.insert(nb, cur);
                    heap.push(Reverse((new_cost, nb)));
                }
            }
        }
        None
    }

    /// A* shortest path from `from` to `to`.
    /// `weight_fn` extracts edge cost, `heuristic` estimates remaining cost to target.
    /// Returns (total_cost, path) or None if unreachable.
    pub fn astar(
        &self,
        from: id::N,
        to: id::N,
        weight_fn: impl Fn(E::Slot, &E::Val) -> Option<u32>,
        heuristic: impl Fn(id::N) -> u32,
    ) -> Option<(u32, Vec<id::N>)> {
        use std::cmp::Reverse;
        use std::collections::{BinaryHeap, HashMap};

        let mut g_score: HashMap<id::N, u32> = HashMap::new();
        let mut prev: HashMap<id::N, id::N> = HashMap::new();
        let mut heap: BinaryHeap<Reverse<(u32, u32, id::N)>> = BinaryHeap::new(); // (f, g, node)

        g_score.insert(from, 0);
        heap.push(Reverse((heuristic(from), 0, from)));

        while let Some(Reverse((_f, g, cur))) = heap.pop() {
            if cur == to {
                let mut path = vec![to];
                let mut n = to;
                while let Some(&p) = prev.get(&n) {
                    path.push(p);
                    n = p;
                }
                path.reverse();
                return Some((g, path));
            }
            if g > *g_score.get(&cur).unwrap_or(&u32::MAX) { continue; }
            let Some(nbs) = self.neighbors(cur) else { continue };
            for (nb, slot, ev) in nbs {
                let Some(w) = weight_fn(slot, ev) else { continue };
                let new_g = g + w;
                if new_g < *g_score.get(&nb).unwrap_or(&u32::MAX) {
                    g_score.insert(nb, new_g);
                    prev.insert(nb, cur);
                    let f = new_g + heuristic(nb);
                    heap.push(Reverse((f, new_g, nb)));
                }
            }
        }
        None
    }

    /// Lazy DFS/BFS path search. Returns an iterator yielding one path at a time.
    pub fn path_search<'a, F: Fn(E::Slot, &E::Val) -> bool + 'a>(
        &'a self,
        from: id::N,
        to: id::N,
        edge_pred: F,
        config: crate::search::path::Config<(), crate::search::path::Traversal>,
        constraint: &'a crate::search::path::PathConstraint,
    ) -> impl Iterator<Item = Vec<id::N>> + 'a {
        crate::search::path::execute_owned(self, from, to, edge_pred, config, constraint)
    }

    /// Lazy navigated path search (Dijkstra, A*, custom). Yields (cost, path).
    pub fn path_navigate<'a, F: Fn(E::Slot, &E::Val) -> bool + 'a>(
        &'a self,
        from: id::N,
        to: id::N,
        edge_pred: F,
        config: crate::search::path::Config<(), crate::search::path::Navigated, E::Val>,
        constraint: &'a crate::search::path::PathConstraint,
    ) -> impl Iterator<Item = (f64, Vec<id::N>)> + 'a {
        crate::search::path::execute_nav_owned(self, from, to, edge_pred, config, constraint)
    }

    pub fn is_adjacent(&self, n1: Id, n2: Id) -> bool {
        self.nodes.get_node(id::N(n1))
            .map(|n| n.adj.contains(id::N(n2)))
            .unwrap_or(false)
    }

    pub(crate) fn get_edge_val(&self, e: E::Def) -> Option<&E::Val> {
        let (nr, slot) = e.into();
        self.edges_between(*nr.n1(), *nr.n2())
            .find(|(s, _)| *s == slot)
            .map(|(_, v)| v)
    }

    pub(crate) fn edges_between(&self, n1: id::N, n2: id::N) -> impl Iterator<Item = (E::Slot, &E::Val)> + '_ {
        self.nodes.get_node(n1)
            .into_iter()
            .flat_map(move |node| {
                node.adj.edges_to(n2).map(move |eid| {
                    let rec = self.edges.store[*eid as usize].as_ref().unwrap();
                    (rec.slot, &rec.val)
                })
            })
    }

    pub(crate) fn edge_ids_between(&self, n1: id::N, n2: id::N) -> impl Iterator<Item = id::E> + '_ {
        self.nodes.get_node(n1)
            .into_iter()
            .flat_map(move |node| node.adj.edges_to(n2))
    }

    pub(crate) fn degrees_insert(&mut self, node: id::N, degree: Id) {
        let pos = self.degrees.partition_point(|&(d, _)| d > degree);
        if pos < self.degrees.len() && self.degrees[pos].0 == degree {
            self.degrees[pos].1.insert(&node);
        } else {
            let mut set = IdSet::<id::N>::default();
            set.insert(&node);
            self.degrees.insert(pos, (degree, set));
        }
    }

    pub(crate) fn degrees_remove(&mut self, node: id::N, degree: Id) {
        let pos = self.degrees.partition_point(|&(d, _)| d > degree);
        if pos < self.degrees.len() && self.degrees[pos].0 == degree {
            self.degrees[pos].1.remove(&node);
            if self.degrees[pos].1.is_empty() {
                self.degrees.remove(pos);
            }
        }
    }

    pub(crate) fn degrees_move(&mut self, node: id::N, old_degree: Id, new_degree: Id) {
        if old_degree == new_degree {
            return;
        }
        self.degrees_remove(node, old_degree);
        self.degrees_insert(node, new_degree);
    }

    pub(crate) fn nodes_with_min_degree(&self, min_degree: Id) -> impl Iterator<Item = &IdSet<id::N>> {
        let end = self.degrees.partition_point(|&(d, _)| d >= min_degree);
        self.degrees[..end].iter().map(|(_, set)| set)
    }

    pub fn watched<'a, W: crate::watch::Watcher<NV, E>>(
        &'a mut self,
        watcher: &'a mut W,
    ) -> crate::watch::WatchedGraph<'a, NV, E, W> {
        crate::watch::WatchedGraph { graph: self, watcher }
    }

    pub(crate) fn build_degrees(&mut self) {
        self.degrees.clear();
        let entries: Vec<(id::N, Id)> = self.nodes.nodes_iter()
            .map(|(n, node)| (n, node.adj.len()))
            .collect();
        for (n, deg) in entries {
            self.degrees_insert(n, deg);
        }
    }

}

impl<NV, E: Edge> super::Graph<NV, E> for MGraph<NV, E> {
    type NodeAdj<'a> = super::AdjIds<'a> where Self: 'a;

    #[inline(always)]
    fn node_count(&self) -> usize {
        self.nodes.len()
    }

    #[inline(always)]
    fn edge_count(&self) -> usize {
        self.edges.len()
    }

    #[inline(always)]
    fn id_space_len(&self) -> usize {
        self.nodes.store.len()
    }

    #[inline(always)]
    fn has_node(&self, n: id::N) -> bool {
        self.nodes.has(n)
    }

    #[inline(always)]
    fn node_val(&self, n: id::N) -> Option<&NV> {
        self.nodes.get(n)
    }

    #[inline(always)]
    fn degree(&self, n: id::N) -> Option<Id> {
        self.nodes.degree(n)
    }

    #[inline(always)]
    fn neighbors<'a>(&'a self, n: id::N) -> Option<impl Iterator<Item = (id::N, E::Slot, &'a E::Val)> + 'a>
    where
        E::Val: 'a,
    {
        MGraph::neighbors(self, n)
    }

    #[inline(always)]
    fn neighbor_ids(&self, n: id::N) -> Option<super::AdjIds<'_>> {
        self.nodes.neighbor_ids(n)
    }

    #[inline(always)]
    fn is_adjacent(&self, n1: Id, n2: Id) -> bool {
        MGraph::is_adjacent(self, n1, n2)
    }

    #[inline(always)]
    fn get_edge_val(&self, e: E::Def) -> Option<&E::Val> {
        MGraph::get_edge_val(self, e)
    }

    #[inline(always)]
    fn edges_between<'a>(&'a self, n1: id::N, n2: id::N) -> impl Iterator<Item = (E::Slot, &'a E::Val)> + 'a
    where
        E::Val: 'a,
    {
        MGraph::edges_between(self, n1, n2)
    }

    #[inline(always)]
    fn iter_edges<'a>(&'a self) -> impl Iterator<Item = (id::N, id::N, E::Slot, &'a E::Val)> + 'a
    where
        E::Val: 'a,
    {
        self.edges.store.iter().filter_map(|opt| {
            opt.as_ref().map(|rec| (rec.n1, rec.n2, rec.slot, &rec.val))
        })
    }

    #[inline(always)]
    fn iter_node_ids(&self) -> impl Iterator<Item = id::N> + '_ {
        self.nodes.iter().map(|(n, _)| n)
    }

    #[inline(always)]
    fn iter_nodes<'a>(&'a self) -> impl Iterator<Item = (id::N, &'a NV, super::AdjIds<'a>)> + 'a
    where
        NV: 'a,
    {
        self.nodes.nodes_iter().map(|(n, node)| (n, &node.val, node.adj.iter()))
    }

    #[inline(always)]
    fn rel(&self, ns: impl Into<NR<id::N>>) -> E::Rel<'_>
    where
        E: HasRel,
    {
        MGraph::rel(self, ns)
    }
}

impl<NV: Clone, E: Edge> MGraph<NV, E>
where
    E::Val: Clone,
{
    /// Rebuilds an `MGraph` from any `Graph` read surface.
    ///
    /// Node ids are preserved slot for slot: every id below `id_space_len`
    /// that the source does not hold stays vacant here and re-enters the free
    /// list, so ids keep their meaning across the conversion. Edge ids are
    /// **not** preserved — the trait exposes no edge slot, so edges are
    /// numbered `0..edge_count` in `iter_edges` order and any edge tombstone
    /// the source held is compacted away.
    ///
    /// Required invariants: the source `Graph` must yield node ids all `< id_space_len`
    /// and edges referencing only live nodes. Violations panic (via indexing/expect),
    /// by design — no fallback or error path exists.
    pub fn from_graph<G: super::Graph<NV, E>>(g: &G) -> Self {
        let mut store: Vec<Option<Node<NV>>> = Vec::new();
        store.resize_with(g.id_space_len(), || None);
        let mut free_ids = IdSpace::default();
        let mut count = 0usize;
        for n in g.iter_node_ids() {
            let val = g.node_val(n).expect("live node id has a value").clone();
            store[*n as usize] = Some(Node::new(val));
            free_ids.remove_id(*n);
            count += 1;
        }
        let mut nodes = Nodes { store, free_ids, count };

        let mut edge_store: Vec<Option<EdgeRec<E::Slot, E::Val>>> = Vec::new();
        let mut edge_free_ids = IdSpace::default();
        for (n1, n2, slot, val) in g.iter_edges() {
            let e = id::E(edge_free_ids.pop_id().expect("edge id space exhausted"));
            if n1 == n2 {
                nodes.get_node_mut(n1).expect("edge endpoint is a live node").adj.insert(n1, e);
            } else {
                let [a, b] = nodes.get_two_mut(n1, n2);
                a.expect("edge endpoint is a live node").adj.insert(n2, e);
                b.expect("edge endpoint is a live node").adj.insert(n1, e);
            }
            edge_store.push(Some(EdgeRec { n1, n2, slot, val: val.clone() }));
        }
        let edge_count = edge_store.len();
        let edges = Edges { store: edge_store, free_ids: edge_free_ids, count: edge_count };

        let mut built = MGraph { nodes, edges, degrees: Vec::new() };
        built.build_degrees();
        built
    }

    pub fn to_vecs(&self) -> (Vec<(Id, NV)>, Vec<(E::Def, E::Val)>) {
        let nodes = self.nodes.iter().map(|(n, v)| (*n, v.clone())).collect();
        let edges = self.edges.iter().map(|(def, val)| (def, val.clone())).collect();
        (nodes, edges)
    }
}

impl<NV, E: Edge> From<MGraph<NV, E>> for (Vec<(Id, NV)>, Vec<(E::Def, E::Val)>) {
    fn from(g: MGraph<NV, E>) -> Self {
        let MGraph { nodes, edges, degrees: _ } = g;
        let nodes = nodes.store.into_iter().enumerate()
            .filter_map(|(i, opt)| opt.map(|node| (i as Id, node.val)))
            .collect();
        let edges = edges
            .store
            .into_iter()
            .filter_map(|opt| {
                opt.map(|rec| (E::edge(rec.slot, (*rec.n1, *rec.n2)), rec.val))
            })
            .collect();
        (nodes, edges)
    }
}

impl<NV, E: HasRel> MGraph<NV, E> {
    pub fn rel(&self, ns: impl Into<NR<id::N>>) -> E::Rel<'_> {
        let nr = ns.into();
        E::rel(nr, self.edges_between(*nr.n1(), *nr.n2()))
    }
    pub fn rel_mut(&mut self, ns: impl Into<NR<id::N>>) -> E::RelMut<'_> {
        let nr = ns.into();
        let n1 = *nr.n1();
        let n2 = *nr.n2();
        let MGraph { nodes, edges, .. } = self;
        let eids: smallvec::SmallVec<[id::E; 4]> = nodes.get_node(n1)
            .map(|node| node.adj.edges_to(n2).collect())
            .unwrap_or_default();
        E::rel_mut(nr, eids.into_iter().map(|eid| {
            let idx = *eid as usize;
            let rec_ref = unsafe { &mut *edges.store.as_mut_ptr().add(idx) };
            let rec = rec_ref.as_mut().unwrap();
            (rec.slot, &mut rec.val)
        }))
    }
}

impl<NV, E: Edge> Default for MGraph<NV, E>
where
    NV: Default,
    E::Val: Default,
{
    fn default() -> Self {
        Self {
            nodes: Nodes::default(),
            edges: Edges::default(),
            degrees: Vec::new(),
        }
    }
}
