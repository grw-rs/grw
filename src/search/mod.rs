...
mod tests {
    ...
    fn bound_path_edge_multihop() {
        ...
        eprintln!("has_paths={} edge_count={} adj_len_0={} adj_len_1={}",
            query.has_paths,
            query.edges.len(),
            query.adj[0].len(),
            query.adj[1].len(),
    ...
    fn bound_path_edge_with_typed_pred() {
        ...
        let q = bound.query();
        eprintln!("pred_test: has_paths={} edge_pred_some={} has_predicates={}",
            q.has_paths,
            q.edge_preds.iter().any(|p| p.is_some()),
            q.has_predicates,
        );
        let bindings = bound.bindings().to_vec();
    ...
    fn homo_cycle_via_path() {
        ...
        // Homo defines terminals, Mono references them for path.
        // Nodes keep Homo morphism (defined there) — n() in Mono doesn't tighten.
        let pattern = search![<(), ER>;
            get(Morphism::Homo) { N(0), N(1) },
            get(Morphism::Mono) { n(0) ^ ..n(1).dfs().len(2..) }
        ];
        let Search::Resolved(r) = pattern.unwrap()
        ...
        let same_node: Vec<_> = matches.iter()
            ...
            .collect();
        assert!(!same_node.is_empty(), "Homo should allow both terminals to bind to same node (cycle)");
    }

    #[test]
    fn navigate_in_search_binds_cheapest_path() {
        ...
        // `.navigate()` now works inside `search!` — the bound path is cost-optimal.
        let pattern = search![<(), W>;
            get(Morphism::Homo) {
                X(0) ^ ..X(1).navigate(Dijkstra::weighted(|w: &u32| *w as f64))
            }
        ];
        let Search::Unresolved(u) = pattern.unwrap() else { panic!("expected context nodes") };
        let bound = u.bind(&[(0, 0), (1, 3)]).unwrap();
        let bindings = bound.bindings().to_vec();
        let query = bound.query();
        let matches: Vec<_> = Seq::search_bound(query, &target, bindings).collect();
        ...
        assert_eq!(p, &[id::N(0), id::N(1), id::N(3)],
            "navigate binds the cheapest path (cost 2), got {:?}", p);
    }
