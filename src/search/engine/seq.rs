...
impl<R: ReverseLookup> State<R> {
                    ...
                    if can_fast_verify {
                        ...
                        if EP::HAS_PREDICATES {
                            for &(neighbor_idx, slot, negated, edge_idx) in &ctx.query.adj_pred[pattern_idx] {
                                ...
                                if ctx.query.has_paths && ctx.query.edges[edge_idx].path.is_some() {
                                    ...
                                    let has_path = crate::search::path::execute_paths(
                                        ...
                                        &edge_filter, path_config, &pc,
                                    ).next().is_some();
                                    let result = if negated { !has_path } else { has_path };
                                    if !result { return false; }
                                } else if !ctx.index.check_edge(raw, mapped_raw, slot, any_slot, negated, ctx.query.edge_preds[edge_idx].as_deref()) {
                                    return false;
                                }
                            }
                        }
    ...
    >(&mut self, ctx: &Ctx<'a, NV, ER, I>, watcher: &mut W) -> (Option<Match>, usize) {
        ...
        if search_len == 0 {
            ...
            if ban_ok {
                return if EM::COUNT_ONLY { (None, 1) } else { (Some(Match { bindings: Vec::new(), path_alts: Vec::new(), path_cursors: Vec::new(), path_edge_indices: Vec::new() }), 0) };
            }
            return (None, 0);
        }
        ...
        loop {
            ...
            if depth + 1 == search_len {
                ...
                let mut path_intermediates: Vec<u32> = Vec::new();
                if BP::ACTIVE && ctx.query.has_paths {
                    for (ei, pe) in ctx.query.edges.iter().enumerate() {
                        if pe.path.is_none() { continue; }
                        let src_raw = self.mapping[pe.source];
                        let tgt_raw = self.mapping[pe.target];
                        if src_raw == super::UNMAPPED || tgt_raw == super::UNMAPPED { continue; }
                        let slot = pe.slot;
                        ...
                        if let Some(path) = crate::search::path::execute_paths(
                            ...
                            &edge_filter, pe.path.as_ref().unwrap(), &pc,
                        ).next() {
                            for &n in &path[1..path.len().saturating_sub(1)] {
                                let raw = *n;
                                if self.reverse.get(raw) == super::UNMAPPED {
                                    self.reverse.set(raw, super::UNMAPPED.wrapping_sub(1));
                                    path_intermediates.push(raw);
                                }
                            }
                        }
                    }
                ...
                for &raw in &path_intermediates {
                    self.reverse.clear(raw);
                }

                if ban_ok && (!ctx.query.has_surjective || self.check_surjective(ctx)) {
                    if EM::COUNT_ONLY {
                        total += 1;
                    } else {
                        let result = self.build_match(ctx);
                        if W::ACTIVE {
                            let control = watcher.on_match(&result.bindings);
                            let local_id = ctx.query.nodes[pattern_idx].local_id;
                            watcher.on_unbind(depth, local_id);
                            self.mapping[pattern_idx] = super::UNMAPPED;
                            self.reverse.clear(*candidate);
