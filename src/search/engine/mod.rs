...
#[derive(Debug)]
pub struct Match {
    bindings: Vec<(LocalId, id::N)>,
    /// All path alternatives per path edge. Outer index = edge index.
    /// Inner vec = list of alternative paths. Each path = full node sequence.
    path_alts: Vec<Vec<Vec<id::N>>>,
    /// Current cursor per path edge — indexes into path_alts.
    path_cursors: Vec<usize>,
    /// Indices of edges that have path alternatives.
    path_edge_indices: Vec<usize>,
}

impl Match {
    pub fn get(&self, pattern_local: impl Into<LocalId>) -> Option<id::N> {
        let lid = pattern_local.into();
        self.bindings.binary_search_by_key(&lid.0, |(l, _)| l.0)
            .ok()
    ...
    /// Current path for a given edge index. Returns the full node sequence
    /// (including start and end) or empty if the edge is not a path edge.
    pub fn path(&self, edge_idx: usize) -> &[id::N] {
        let alts = match self.path_alts.get(edge_idx) {
            Some(a) if !a.is_empty() => a,
            _ => return &[],
        };
        let cursor = self.path_cursors.get(edge_idx).copied().unwrap_or(0);
        alts.get(cursor).map(|v| v.as_slice()).unwrap_or(&[])
    }

    /// Number of path alternatives for a given edge index.
    pub fn path_count(&self, edge_idx: usize) -> usize {
        self.path_alts.get(edge_idx).map(|a| a.len()).unwrap_or(0)
    }

    /// Advance to the next path combination. Returns true if a new
    /// combination is available, false if exhausted.
    /// Iterates rightmost path edge first (odometer style).
    /// When `injective` is true (Mono morphism), skips combinations where
    /// intermediate path nodes overlap across different paths.
    pub fn next_paths(&mut self) -> bool {
        self.advance_paths(false)
    }

    /// Like next_paths but enforces that intermediate nodes across
    /// all paths are mutually distinct (for Mono/SubIso morphisms).
    pub fn next_paths_injective(&mut self) -> bool {
        self.advance_paths(true)
    }

    fn advance_paths(&mut self, injective: bool) -> bool {
        if self.path_edge_indices.is_empty() { return false; }

        loop {
            // Odometer advance
            let mut advanced = false;
            for &ei in self.path_edge_indices.iter().rev() {
                let cursor = &mut self.path_cursors[ei];
                *cursor += 1;
                if *cursor < self.path_alts[ei].len() {
                    advanced = true;
                    break;
                }
                *cursor = 0;
            ...
            if !injective { return true; }

            // Check cross-path injectivity: no shared intermediate nodes
            if self.paths_are_injective() { return true; }
            // Otherwise loop to try next combination
        }
    }

    pub fn paths_are_injective(&self) -> bool {
        let mut seen = std::collections::HashSet::new();
        for &ei in &self.path_edge_indices {
            let path = &self.path_alts[ei][self.path_cursors[ei]];
            // Intermediates = path without first and last (endpoints are in bindings)
            if path.len() > 2 {
    ...
    /// Reset path cursors to the beginning.
    pub fn reset_paths(&mut self) {
        for c in &mut self.path_cursors { *c = 0; }
    }

    #[doc(hidden)]
    pub fn with_paths(path_alts: Vec<Vec<Vec<id::N>>>) -> Self {
        let path_edge_indices: Vec<usize> = path_alts.iter().enumerate()
            .filter(|(_, a)| !a.is_empty())
            .map(|(i, _)| i)
            .collect();
        let path_cursors = vec![0; path_alts.len()];
        Match { bindings: Vec::new(), path_alts, path_cursors, path_edge_indices }
    }
...
impl std::ops::Index<LocalId> for Match {
    ...
    fn index(&self, lid: LocalId) -> &id::N {
        let i = self.bindings.binary_search_by_key(&lid.0, |(l, _)| l.0)
...
impl<R: ReverseLookup> State<R> {
    ...
    pub(crate) fn is_feasible<NV, ER: graph::Edge, I: Index<NV, ER>, W: crate::watch::Watcher<NV, ER>>(&self, ctx: &Ctx<'_, NV, ER, I>, pattern_idx: usize, candidate: id::N, watcher: &mut W) -> bool {
        ...
        for &(neighbor_idx, slot, negated, edge_idx) in &ctx.query.adj[pattern_idx] {
            ...
            if mapped_id != UNMAPPED {
                let result = if ctx.query.edges[edge_idx].path.is_some() {
                    // Path edge: run path executor instead of single-edge check
                    ...
                    let has_path = crate::search::path::execute_paths(
                        ctx.target,
                        id::N(candidate_id as crate::Id),
                        id::N(mapped_id as crate::Id),
                        &edge_filter,
                        path_config,
                        &pc,
                    ).next().is_some();
                    if effective_negated { !has_path } else { has_path }
                } else {
                    ctx.index.check_edge(candidate_id, mapped_id, slot, any_slot, effective_negated, ctx.query.edge_preds[edge_idx].as_deref())
                };
                if W::ACTIVE {
    ...
    pub(crate) fn best_mapped_neighbor<NV, ER: graph::Edge, I: Index<NV, ER>>(&self, ctx: &Ctx<'_, NV, ER, I>, pattern_idx: usize) -> Option<id::N> {
        ...
        if pattern_idx < 64 && !ctx.query.node_has_predicates[pattern_idx] && !ctx.query.has_paths {
            ...
            for &(neighbor_idx, _slot, negated, edge_idx) in &ctx.query.adj[pattern_idx] {
                ...
                if ctx.query.edges[edge_idx].path.is_some() {
                    continue;
                }
    ...
    pub(crate) fn build_match<NV, ER: graph::Edge, I: Index<NV, ER>>(&self, ctx: &Ctx<'_, NV, ER, I>) -> Match {
        ...
        // Collect all path alternatives for each path edge
        let mut path_alts = Vec::new();
        let mut path_edge_indices = Vec::new();
        if ctx.query.has_paths {
            for (ei, pe) in ctx.query.edges.iter().enumerate() {
                if let Some(ref path_config) = pe.path {
                    let src_raw = self.mapping[pe.source];
                    let tgt_raw = self.mapping[pe.target];
                    if src_raw != UNMAPPED && tgt_raw != UNMAPPED {
                        let slot = pe.slot;
                        ...
                        if !found.is_empty() {
                            path_edge_indices.push(ei);
                        }
                        path_alts.push(found);
                    } else {
                        path_alts.push(Vec::new());
                    }
                } else {
                    path_alts.push(Vec::new());
                }
        ...
        let path_cursors = vec![0; path_alts.len()];
        Match { bindings: pairs, path_alts, path_cursors, path_edge_indices }
    }
}
