...
impl<NV, ER: graph::Edge> Flattener<NV, ER> {
    ...
    fn flatten_and_collect(
        ...
        cluster_defined: &mut FxHashSet<usize>,
        cluster_edges: &mut Vec<usize>,
        cluster_morphism: Morphism,
    ) -> Result<usize, error::Search> {
        let (local_id, kind, negated, node_pred, edges, exist_target, is_def) = match op {
            Op::Free { id, node_pred, negated, edges, .. } => {
                let lid = match id {
                    Some(lid) => lid,
                    None => self.fresh_anon(),
                };
                (lid, NodeKind::New, negated, node_pred, edges, None, true)
            }
            Op::FreeRef { id, edges } => (id, NodeKind::New, false, None, edges, None, false),
            Op::Exist { id, node_pred, negated, edges } => {
                let lid = LocalId(*id);
                (lid, NodeKind::Exist, negated, node_pred, edges, Some(id), true)
            }
            Op::ExistRef { id, edges } => {
                let lid = LocalId(*id);
                (lid, NodeKind::Exist, false, None, edges, Some(id), false)
            }
            Op::Context { id, node_pred, negated, edges } => {
                (id, NodeKind::Translated, negated, node_pred, edges, None, true)
            }
            Op::ContextRef { id, edges } => (id, NodeKind::Translated, false, None, edges, None, false),
        };

        let node_idx = self.ensure_node(local_id, kind, negated);
        cluster_nodes.insert(node_idx);
        if is_def { cluster_defined.insert(node_idx); }
        ...
        for edge_op in edges {
            let target_idx = self.flatten_and_collect(
                edge_op.target, cluster_nodes, cluster_defined, cluster_edges, cluster_morphism,
            )?;
...
pub fn compile<NV, ER: graph::Edge>(
    ...
    for cluster in get_cluster_ops {
        ...
        let mut defined_indices = FxHashSet::default();
        let mut edge_indices = Vec::new();

        for op in cluster.ops {
            flat.flatten_and_collect(op, &mut node_indices, &mut defined_indices, &mut edge_indices, cluster.morphism)?;
        }
        ...
        clusters.push(Cluster {
            ...
            defined_indices: {
                let mut v: Vec<usize> = defined_indices.into_iter().collect();
                v.sort_unstable();
                v
            },
        });
    ...
    for cluster in &clusters {
        if cluster.decision == Decision::Get {
            for &ni in &cluster.defined_indices {
    ...
    let has_paths = flat.edges.iter().any(|e| e.path.is_some());
