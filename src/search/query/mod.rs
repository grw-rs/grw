...
pub(crate) struct PatternEdge<ER: graph::Edge> {
    ...
    pub(crate) path: Option<super::path::Config<(), super::path::Unset, ER::Val>>,
    pub(crate) path_morphism: Morphism,
}
...
pub(crate) struct Cluster {
    ...
    pub(crate) defined_indices: Vec<usize>,
}
...
mod tests {
    ...
    #[test]
    fn compile_node_morphism_ref_does_not_tighten() {
        // N(0) defined in Mono. n(0) referenced in SubIso — should NOT tighten.
        let Search::Resolved(r): Search<(), ER> = crate::search![
            get(Morphism::Mono) {
                N(0) ^ N(1)
            },
        ...
        let q = r.query();
        assert_eq!(morphism_for(q, 0), Morphism::Mono); // defined in Mono, ref in SubIso doesn't tighten
    ...
    fn compile_node_morphism_three_clusters() {
        // N(0) defined in Homo. n(0) referenced in Mono and SubIso — keeps Homo.
        let Search::Resolved(r): Search<(), ER> = crate::search![
            get(Morphism::Homo) {
                N(0) ^ N(1)
            },
        ...
        let q = r.query();
        assert_eq!(morphism_for(q, 0), Morphism::Homo); // defined in Homo, refs don't tighten
