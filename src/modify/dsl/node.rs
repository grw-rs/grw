...
macro_rules! impl_undir_new_op {
    ($NodeTy:ty, $V:ident, $Dir:ident, $_SlotDir:ident, $Op:ident, $op:ident) => {
        impl<NV, $V, EV: IntoVal<ER::Val> + IsFullVal, ER: graph::Edge + $Dir, RHS: IntoNode<NV, ER>>
    ...
    ($NodeTy:ty, $Dir:ident, $_SlotDir:ident, $Op:ident, $op:ident) => {
        impl<NV, EV: IntoVal<ER::Val> + IsFullVal, ER: graph::Edge + $Dir, RHS: IntoNode<NV, ER>>
            $Op<RHS> for UndirPending<$NodeTy, edge::new::Edge<EV, NV, ER>>
        {
            type Output = $NodeTy;
            fn $op(self, rhs: RHS) -> $NodeTy {
...
// HasRawVal variants for new edges: Node & E().val(slot_val) >> / << / ^ Target
macro_rules! impl_rawval_undir_new_op {
    ($NodeTy:ty, $V:ident, $Dir:ident, $SlotDir:ident, $Op:ident, $op:ident) => {
        impl<NV, $V, V_, ER: graph::Edge + $Dir + SlotVal<$SlotDir, SlotType = V_>, RHS: IntoNode<NV, ER>>
            $Op<RHS> for UndirPending<$NodeTy, edge::new::Edge<HasRawVal<V_>, NV, ER>>
        {
            type Output = $NodeTy;
            fn $op(self, rhs: RHS) -> $NodeTy {
                let mut node = self.0;
    ...
    ($NodeTy:ty, $Dir:ident, $SlotDir:ident, $Op:ident, $op:ident) => {
        impl<NV, V_, ER: graph::Edge + $Dir + SlotVal<$SlotDir, SlotType = V_>, RHS: IntoNode<NV, ER>>
            $Op<RHS> for UndirPending<$NodeTy, edge::new::Edge<HasRawVal<V_>, NV, ER>>
        {
            type Output = $NodeTy;
            fn $op(self, rhs: RHS) -> $NodeTy {
...
for_each_dir!(impl_rawval_undir_new_op!(new::Node<NV, V, ER>, V));
for_each_dir!(impl_rawval_undir_new_op!(exist::Node<NV, V, ER>, V));
for_each_dir!(impl_rawval_undir_new_op!(translated::Node<NV, V, ER>, V));
for_each_dir!(impl_rawval_undir_new_op!(new::Ref<NV, ER>));
...
macro_rules! impl_undir_exist_op {
    ($NodeTy:ty, $V:ident, $Dir:ident, $_SlotDir:ident, $Op:ident, $op:ident) => {
        impl<NV, $V, EV: IntoOptional<ER::Val> + IsFullVal, ER: graph::Edge + $Dir, RHS: IntoExistNode<NV, ER>>
    ...
    ($NodeTy:ty, $Dir:ident, $_SlotDir:ident, $Op:ident, $op:ident) => {
        impl<NV, EV: IntoOptional<ER::Val> + IsFullVal, ER: graph::Edge + $Dir, RHS: IntoExistNode<NV, ER>>
            $Op<RHS> for UndirPending<$NodeTy, edge::exist::Edge<EV, NV, ER>>
        {
            type Output = $NodeTy;
            fn $op(self, rhs: RHS) -> $NodeTy {
...
// HasRawVal variants for exist edges: Node & e().val(slot_val) >> / << / ^ ExistTarget
macro_rules! impl_rawval_undir_exist_op {
    ($NodeTy:ty, $V:ident, $Dir:ident, $SlotDir:ident, $Op:ident, $op:ident) => {
        impl<NV, $V, V_, ER: graph::Edge + $Dir + SlotVal<$SlotDir, SlotType = V_>, RHS: IntoExistNode<NV, ER>>
            $Op<RHS> for UndirPending<$NodeTy, edge::exist::Edge<HasRawVal<V_>, NV, ER>>
        {
            type Output = $NodeTy;
            fn $op(self, rhs: RHS) -> $NodeTy {
                let mut node = self.0;
                let op = edge::Exist::Bind(edge::Bind::Swap(ER::wrap_slot_val((self.1).0 .0)));
                node.edges.push(Edge::Exist {
                    slot: <ER as $Dir>::SLOT,
                    op,
                    target: rhs.into_exist_node(),
    ...
    ($NodeTy:ty, $Dir:ident, $SlotDir:ident, $Op:ident, $op:ident) => {
        impl<NV, V_, ER: graph::Edge + $Dir + SlotVal<$SlotDir, SlotType = V_>, RHS: IntoExistNode<NV, ER>>
            $Op<RHS> for UndirPending<$NodeTy, edge::exist::Edge<HasRawVal<V_>, NV, ER>>
        {
            type Output = $NodeTy;
            fn $op(self, rhs: RHS) -> $NodeTy {
...
for_each_dir!(impl_rawval_undir_exist_op!(exist::Node<NV, V, ER>, V));
for_each_dir!(impl_rawval_undir_exist_op!(exist::Ref<NV, ER>));
for_each_dir!(impl_rawval_undir_exist_op!(translated::Node<NV, V, ER>, V));
for_each_dir!(impl_rawval_undir_exist_op!(translated::Ref<NV, ER>));
